...
function main() {
  ...
  /* An owner column that does not exist is the worst kind of wrong here: it
     does not throw, it just makes ownerPermissions() return undefined, so rows
     are created with NO permissions — and Appwrite then defaults them to
     owner-only READ, which makes the post invisible to everyone else. Three
     mappings were wrong (events.created_by, event_forms.created_by,
     inventory_items.user_id) and nothing said so. Now the schema is the judge. */
  const ownProblems = [];
  for (const [table, col] of Object.entries(own.ownerColumn)) {
    if (col === '$id') continue;
    const t = out.tables.find(x => x.$id === table);
    if (!t) { ownProblems.push(`${table}: no such table`); continue; }
    if (!t.columns.some(c => c.key === col)) ownProblems.push(`${table}.${col}: no such column`);
  }
  if (ownProblems.length) {
    console.error('\nownership.json does not match the schema:');
    for (const p2 of ownProblems) console.error('  ! ' + p2);
    console.error('\nA missing owner column silently creates rows nobody can see. Refusing to generate.\n');
    process.exit(1);
  }
